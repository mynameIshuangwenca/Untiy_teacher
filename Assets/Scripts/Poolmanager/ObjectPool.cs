...
public class ObjectPool : MonoSingleton<ObjectPool>
{
    ...
    public GameObject CreateObject(string key, GameObject go, Vector3 position, Transform parent)

    {
        ...
        {
            tempgo = Instantiate(go, position, quaternion, parent); //那就根据传入的预设物，生成一个新物体
            print("实例化物体数量：" + i++);
...
/*  item                    =  ObjectPool.Instance.CreateObject("Object1", go, Vector3.zero, new Quaternion(0, 0, 0, 0));\
     ...
     quaternion     rotation：实例化的对象的旋转坐标（旋转四元数）
